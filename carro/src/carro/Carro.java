...
public class Carro {
	...
	void embarcar(int qtd) {
		if (qtd<0);
			System.out.println("Não pode embarcar número negativo") ;
			return;
	...
	void desembarcar(int qtd) {
		if (qtd<0);
			return;
				System.out.println("Não pode desembarcar número negativo") ;
	
					
		if(this.nPessoas < qtd) {
			System.out.println("Só tem" + this.nPessoas + "no carro") ;
		}
			this.nPessoas -= qtd;
